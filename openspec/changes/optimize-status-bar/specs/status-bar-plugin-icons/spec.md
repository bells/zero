...
The system SHALL render a primary ZTool status item followed by plugin status items for installed, enabled, and status-bar-visible tool plugins.
...
- **THEN** the status bar displays the primary ZTool logo followed by screenshot and caffeine plugin items in deterministic order

#### Scenario: Hidden plugin item is omitted
- **WHEN** the user hides the caffeine item in status bar preferences
- **THEN** the status bar displays the primary ZTool logo and omits the caffeine plugin item
...
- **THEN** the primary ZTool logo remains available as the app entry point
...
- **WHEN** the user hides the screenshot item and restarts ZTool
...
- **THEN** the system hides plugin sub-items while keeping the primary ZTool entry point available
...
- **THEN** it displays a compact preview containing the primary ZTool logo and currently visible plugin items
...
### Requirement: Caffeine status item toggles caffeine mode
...
#### Scenario: Caffeine expires automatically
...
#### Scenario: Caffeine enable fails
...
### Requirement: Screenshot status item starts screenshot flow
...
#### Scenario: Screenshot start fails
...
#### Scenario: Screenshot item respects plugin visibility
...
- **THEN** the system opens a ZTool surface with that plugin selected
...
- **WHEN** ZTool runs on macOS with plugin item display enabled
- **THEN** the system renders separate native status items for the primary ZTool logo and visible plugin items

#### Scenario: Platform uses fallback action row
- **WHEN** ZTool runs on a platform where separate native status items are not practical
