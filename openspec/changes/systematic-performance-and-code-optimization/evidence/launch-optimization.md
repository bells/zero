...
The 5 ms p95 target is a reference-hardware measurement. Shared GitHub Actions runners now report `reference_target_met` without failing on host contention; the 2026-09-25 CI run measured Nucleo/production p95 at 5.408/5.521 ms while the same source measured 4.325/4.297 ms on the local M2. To enforce the target on the documented reference machine, set `ZERO_LAUNCH_REFERENCE_BENCH=1` when running the command above. A hosted-runner result above 5 ms is retained as evidence and requires a matched reference rerun before declaring a regression or target completion.
